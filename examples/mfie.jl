...
Φ = range(0,stop=0,length=1); Θ = range(0,stop=π,length=100)
pts = [point(cos(ϕ)*sin(θ), sin(ϕ)*sin(θ), cos(θ)) for ϕ in Φ for θ in Θ]
F = Maxwell3D.farfield(wavenumber=κ)
ffd = potential(F, pts, u, X)
xs = range(-2,stop=2,length=51); zs = range(-8,stop=4,length=150)
gridpoints = [point(x,0,z) for x in xs, z in zs]
N = MWSingleLayerField3D(wavenumber = κ)
nfd = reshape(potential(N, gridpoints, u, X), size(gridpoints)) .- E.(gridpoints)
